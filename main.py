...
# load flames
dashboard_bg = pygame.transform.scale((pygame.image.load(
    'assets/background/dashboard.jpg').convert_alpha()), (width, height))
setting_bg = pygame.transform.scale((pygame.image.load(
    'assets/background/setting.jpg').convert_alpha()), (width, height))
score_bg = pygame.transform.scale((pygame.image.load(
    'assets/background/score.jpg').convert_alpha()), (width, height))
about_bg = pygame.transform.scale((pygame.image.load(
    'assets/background/aboutbg.jpg').convert_alpha()), (width, height))
help_bg = pygame.transform.scale((pygame.image.load(
    'assets/background/score.jpg').convert_alpha()), (width, height))
star_texture = pygame.transform.scale((pygame.image.load(
    'assets/gamebg/stars_texture.png').convert_alpha()), (width, height))
galaxy = pygame.transform.scale((pygame.image.load(
    'assets/gamebg/galaxy.png').convert_alpha()), (width, height))
bullet_img = pygame.image.load(
    'assets/player/Laser/bullet.gif').convert_alpha()
enemy_bullet_img = pygame.image.load(
    'assets/enemy/Laser/bullet.png').convert_alpha()
firstaid_img = pygame.transform.scale(pygame.image.load(
    'assets/gamebg/first-aid.png').convert_alpha(), (32, 32))
ammo_img = pygame.transform.scale(pygame.image.load(
    'assets/gamebg/ammobg.png').convert_alpha(), (22, 22))
...
def author():
    ...
    screen.blit(text, ((width) - (textwidth + 20),
                (height - (textheight * 2))))
...
class Dashboard:
    ...
    def PlayerShipAndLevel(self):
        ...
        ship = pygame.transform.scale(pygame.image.load(
            self.selectedship).convert_alpha(), (60, 120))
        shiprect = ship.get_rect()
        shiprect.topleft = ((rect.x + (title.get_width() / 2) -
                            (ship.get_width() / 2), rect.y + title.get_height() + 15))

        # showing flame of ship
        flame = pygame.image.load(
            'assets/player/flame/0 (1).png').convert_alpha()
        screen.blit(flame, (shiprect.x + ship.get_width()/2 -
                    flame.get_width()/2, shiprect.y+ship.get_height()-10))
        ...
        level = font.render(
            '  LEVEL : ' + self.selectedlevel+'  ', True, WHITE, DARKBLUE)
    ...
    def ScoreSlider(self):
        ...
        leftarrow = Button(self.sliderboxx-25, self.sliderboxy + (
            box.get_height()/2) - 17, 25, 35, btText, 20, DARKBLUE, RED, hover=False)
        ...
        screen.blit(title, (self.sliderboxx + (box.get_width()/2) -
                    (title.get_width()/2), self.sliderboxy))

        # fetch score data
        headerfont = pygame.font.SysFont('arial', 15, 2)
        header = headerfont.render(
            'INDEX__________LEVEL__________SCORE', True, WHITE)
        screen.blit(header, (self.sliderboxx + (box.get_width()/2) -
                    (header.get_width()/2), self.sliderboxy + header.get_height() + 5))
        ...
        while i >= 0:
            if count <= 10:
                ...
                screen.blit(
                    lvl, (self.sliderboxx + (box.get_width()/2) - (lvl.get_width()/2), posy))

                scr = datafont.render(scorelist[i][1], True, WHITE)
                screen.blit(
                    scr, (self.sliderboxx + (box.get_width()) - (scr.get_width()) - 50, posy))
    ...
    def StartGame(self):
        ...
        screen.blit(score, (width - title.get_width() -
                    score.get_width() - 20, 10))
        ...
        highscore = font.render(
            'HIGHSCORE : '+str(max(scorelist)), True, WHITE)
        screen.blit(highscore, (width - title.get_width() -
                    highscore.get_width() - score.get_width() - 30, 10))

        ship = pygame.transform.rotate(pygame.transform.scale(
            pygame.image.load(self.selectedship), (20, 40)), 90)
        screen.blit(ship, (width - title.get_width() - ship.get_width() -
                    score.get_width() - 40 - highscore.get_width(), 10))
        ...
        self.maprect = self.mapsurf.get_rect(
            topleft=(width-(self.mapsurf.get_width() + 5), 10 + highscore.get_height()))
    ...
    def SettingSec(self):
        ...
        screen.blit(title, ((self.titlex + (self.titlewidth/2) - (title.get_width()/2)),
                    (self.titley + (self.titleheight / 2) - (title.get_height() / 2))))
        ...
        screen.blit(levelboxtitle, ((width/2) -
                    (levelboxtitle.get_width()/2), boxy))
        ...
        if self.selectedlevel == 'LOW':
            ...
            levelbtborder = pygame.surface.Surface((70, 30))
            levelbtborder.fill(RED)
            screen.blit(levelbtborder, (boxx + boxwidth /
                        2 - 35, boxy+(boxheight/2)-25))
        elif self.selectedlevel == 'HIGH':
            levelbtborder = pygame.surface.Surface((70, 30))
            levelbtborder.fill(RED)
            screen.blit(levelbtborder, (boxx + boxwidth -
                        75, boxy+(boxheight/2)-25))

        lowbt = Button(boxx + 10, boxy+(boxheight/2)-20, 60, 20,
                       'LOW', 11, WHITE, YELLOW, DARKBLUE, False)
        mediumbt = Button(boxx + boxwidth/2 - 30, boxy+(boxheight/2)-20,
                          60, 20, 'MEDIUM', 11, WHITE, YELLOW, DARKBLUE, False)
        highbt = Button(boxx + boxwidth-70, boxy+(boxheight/2)-20,
                        60, 20, 'HIGH', 11, WHITE, YELLOW, DARKBLUE, False)
    ...
    def ScoreSec(self):
        ...
        pygame.Surface.set_alpha(box, 150)
        boxrect = box.get_rect()
        boxrect.topleft = (width/2 - box.get_width()/2,
                           height/2 - box.get_height()/2)
        screen.blit(box, boxrect)
        ...
        with open('score.csv', 'r') as scorelist:
            ...
            for score in data:
                ...
                if score_y > (boxrect.y + box.get_height() - scr.get_height()) or score_y < (boxrect.y):
                    ...
                    screen.blit(
                        lvl, (boxrect.x + (box.get_width()/2) - (lvl.get_width()/2), score_y))
                    screen.blit(scr, (boxrect.x + box.get_width() -
                                scr.get_width() - 55, score_y))

                score_y += scr.get_height() + 10
                score_total_height += scr.get_height() + 5
                count += 1
                # print(score)
        btnup = Button(boxrect.x + box.get_width() - 20, boxrect.y,
                       20, 20, '/\\', 14, DARKGREEN, RED, hover=False)
        btndown = Button(boxrect.x + box.get_width() - 20, boxrect.y +
                         box.get_height() - 20, 20, 20, '\\/', 14, DARKGREEN, RED, hover=False)

        # show scroller thumb and scorll it
        scroll = pygame.surface.Surface((5, 20))
        scrollrect = scroll.get_rect()
        scrollrect.topleft = ((boxrect.x + box.get_width()) - 10 -
                              (scroll.get_width()/2), boxrect.y + 20 + self.scoresecscrolly)
        scroll.fill(WHITE)
        ...
        if btnup.draw(screen):
            if scrollrect.y < (boxrect.y + 20):
                ...
                self.scoresecy += 2
                self.scoresecscrolly -= ((box.get_height() - 40)
                                         * 2)/score_total_height
        ...
        lowscore = highscoretitlefont.render(
            'Low         :    '+str(max(low_high_score)), True, WHITE)
        mediumscore = highscoretitlefont.render(
            'Medium   :     '+str(max(medium_high_score)), True, WHITE)
        highscore = highscoretitlefont.render(
            'High        :    '+str(max(high_high_score)), True, WHITE)

        screen.blit(highscoretitle, (((width/4)/2) -
                    highscoretitle.get_width()/2, boxrect.y))

        screen.blit(lowscore, (45, boxrect.y +
                    highscoretitle.get_height() + 15))
        screen.blit(mediumscore, (45, boxrect.y +
                    highscoretitle.get_height() + 30 + lowscore.get_height()))
        screen.blit(highscore, (45, boxrect.y + highscoretitle.get_height() +
                    45 + lowscore.get_height() + mediumscore.get_height()))
    ...
    def AboutSec(self):
        ...
        boxrect = box.get_rect(
            topleft=(width/2 - box.get_width()/2, height/2 - box.get_height()/2))
        box.fill(BLACK)
        ...
        for i in range(len(content)):
            ...
            screen.blit(line, ((line_x + (box.get_width()/2) -
                        (line.get_width()/2)), line_y + 15))
    ...
    def HelpSec(self):
        ...
        surfrect.topleft = (width/2 - surf.get_width()/2,
        ...
        btup = Button(surfrect.x + surf.get_width() - 20, surfrect.y,
                      20, 20, '/\\', 15, LIGHTGREEN, RED, WHITE, False)
        btdown = Button(surfrect.x + surf.get_width() - 20, surfrect.y +
                        surf.get_height() - 20, 20, 20, '\\/', 15, LIGHTGREEN, RED, WHITE, False)
        ...
        scroll = pygame.surface.Surface((5, 20))
        scrollrect = scroll.get_rect()
        scrollrect.topleft = (surfrect.x + surf.get_width() -
                              scroll.get_width()/2 - 10, self.helpmsgscrolly+surfrect.y + 20)
        scroll.fill(WHITE)
        screen.blit(scroll, scrollrect)
    ...
    def ChooseShipSec(self):
        ...
        screen.blit(surf, (width/2 - ((width - 100)/2),
                    height/2 - ((height - 100)/2)))

        # Note : choose space ship by clicking onit
        note = font.render(
            'Note : choose space-ship by clicking on it !', True, WHITE)
        pygame.Surface.set_alpha(note, 150)
        screen.blit(note, (width / 2 - note.get_width() /
                    2, height - note.get_height() - 10))
        ...
        for i in range(3):
            ...
            for j in range(1, 4):
                ...
                scaleimg = pygame.transform.scale(
                    img, (int(img.get_width() * 0.5), int(img.get_height() * 0.5)))
                imgrect = scaleimg.get_rect()
                imgrect.topleft = (x, y)

                # draw border on selected image
                if selectimg == self.selectedship:
                    borderbox = pygame.surface.Surface(
                        (scaleimg.get_width()+10, scaleimg.get_height()+10))
                ...
                if imgrect.collidepoint(pos):
                    hoverbox = pygame.surface.Surface(
                        (scaleimg.get_width()+10, scaleimg.get_height()+10))
                    hoverbox.fill(YELLOW)
                    screen.blit(hoverbox, (imgrect.x - 5, imgrect.y - 5))
                    # display image on big size
                    bigimg = pygame.transform.scale(
                        img, (int(img.get_width() * 1.3), int(img.get_height() * 1.3)))
                    screen.blit(bigimg, ((width/2 - ((width - 100)/2)) + (width - 100) - bigimg.get_width(
                    ) - 20, ((height/2 - ((height - 100)/2)) + ((height-100)/2) - bigimg.get_height() / 2)))
...
class Planet(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.flames = []
        for i in range(1, 61):
            self.flames.append(pygame.transform.scale(
                pygame.image.load('assets/gamebg/Moon/'+str(i)+'.png'), (90, 90)))
        self.index = 0
...
class Player(pygame.sprite.Sprite):
        ...
        if self.cool_down == 0:
            ...
            if self.ammo > 0:
                bullet = Bullet(self.rect.centerx,
                                self.rect.centery, bullet_img, 'UP')
    ...
    def HealthBar(self):
        ...
        screen.blit(showhealth, (50 + (greensurf.get_width()/2) - (showhealth.get_width()/2),
                    15 + (greensurf.get_height()/2) - (showhealth.get_height()/2)))
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, level):
        ...
        self.one = pygame.transform.rotate(pygame.transform.scale(pygame.image.load(
            'assets/enemy/Ships/enemy-1.png').convert_alpha(), (35, 70)), 180)
        self.two = pygame.transform.rotate(pygame.transform.scale(pygame.image.load(
            'assets/enemy/Ships/enemy-2.png').convert_alpha(), (35, 70)), 180)
        self.three = pygame.transform.rotate(pygame.transform.scale(pygame.image.load(
            'assets/enemy/Ships/enemy-3.png').convert_alpha(), (35, 70)), 180)
    ...
    def update(self):
        ...
        if self.health >= 0:
            ...
            screen.blit(
                redrect, (self.rect.x, self.rect.y - redrect.get_height()))
            screen.blit(greenrect, (self.rect.x,
                        self.rect.y - greenrect.get_height()))
...
while run:
    ...
    # open restart section
        ...
        screen.fill(BLACK)
        db.RestartSec()
        restartbt = Button(width/2 - 60, 130, 120, 25, 'Restart',
                           18, DARKBLUE, LIGHTGREEN, hover=False)
        exitbt = Button(width/2 - 60, 180, 120, 25, 'Exit',
                        18, DARKBLUE, LIGHTGREEN, hover=False)
    ...
    # it means game is started
        ...
        # show flame of the player
        particles.append([[player.rect.x+25, player.rect.y+100], [
                         random.randint(0, 15)/10 - 1, random.randint(0, 2)], random.randint(4, 6)])
